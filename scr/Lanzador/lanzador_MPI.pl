...
# 			john@aopcjc.uab.es
# Adapted by: Nicolas Mendez PUJ
# 			nicolas_mendez@javeriana.edu.co
# Mayo 2023
#
# Lanza el binary juntos con los argumentos desde argumentos.txt
# y los eventos, desde el archivo Entrada_Experimentos_MPI
...
foreach $executable (@Binarys) {
	...
	chop($CARP);
	system ("mkdir $Path/../../out/Soluciones_MPI/$CARP");
	foreach $thread (@Threads) {
		foreach $size (@Size) {
			$file = "$Path/../../out/Soluciones_MPI/$CARP/$executable-".$size."-TH-".$thread.".txt";
			print "$executable $size $thread ... on $file\n";
		 	for ($k = 0; $k < $Repet; $k++) {
				system("mpirun  --oversubscribe -np $thread $Path/../../bin/BIN_MPI/$executable $size 0 0  >> $file");
				#print "$Path/../../bin/BIN_MPI/$executable $size $thread 0 0  \n";
			}
			close($file);
		}
	}
