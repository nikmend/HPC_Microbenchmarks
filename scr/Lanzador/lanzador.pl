...
foreach $executable (@Binarys) {
	...
	chop($CARP);
	system ("mkdir $Path../../out/Soluciones/$CARP");
	foreach $thread (@Threads) {
		foreach $size (@Size) {
			$file = "$Path../../out/Soluciones/$CARP/$executable-".$size."-TH-".$thread.".txt";
			print "$executable $size $thread ... on $file\n";
		 	for ($k = 0; $k < $Repet; $k++) {
				system("$Path/../../bin/BIN_OPENMP/$executable $size $thread 0 0  >> $file");
